...
class RegistrationController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $query = ApplicationV2::join('application_v2_profile_user', 'applications_v2.id', '=', 'application_v2_profile_user.application_id')
                ...
                ->where('profile_users.created_by', auth()->id()) 
            ...
            return datatables()->of($query)
                ...
                ->editColumn('vessels_no', function ($row) {
                    return $row->vessels_no ?: '-';
                })
    ...
    public function create()
    {
        ...
        return view('app.vessel.manager.create', [
            'managers' => ProfileUser::where('user_type', $type_vessel_manager)
                ->where('is_active', true)
                ->orderBy('name')
                ->get(),
            'vessels' => Vessel::where('user_id', auth()->id())
            ->orderBy('vessel_no')
            ->get(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $email_rule = Rule::unique('profile_users')->where('user_type', $type_vessel_manager);
        ...
        'icno' => [
            ...
            function ($attribute, $value, $fail) {
                ...
                if ($currentUser->username === $value) {
                    return $fail('Pemilik vesel tidak dibenarkan berdaftar sebagai pengurus vesel untuk vesel sendiri');
                }

                // 2. Check if IC belongs to DOF staff
                $user = \App\Models\User::where('username', $value)->first();
                if ($user && $user->entity_id !== null) {
                    return $fail('Kakitangan DOF tidak dibenarkan berdaftar sebagai pengurus vesel');
                }
            },
        ],
        ...
        'email' => [
            ...
            'email',
            'max:255',
            $email_rule, // your existing rule (e.g., unique check)
            function ($attribute, $value, $fail) {
                $currentUser = auth()->user();

                // 4. Email is same as current user
                if ($currentUser->email === $value) {
                    return $fail('Emel pemilik vesel tidak boleh didaftarkan dibawah pengurus vesel');
                }

                // 5. Email belongs to DOF staff
                $user = \App\Models\User::where('email', $value)->first();
                if ($user && $user->entity_id !== null) {
                    return $fail('Emel kakitangan DOF tidak boleh didaftarkan dibawah pengurus vesel');
                }
            },
        ],
        ...
        try {
            ...
            if ($vessel_array && is_array($vessel_array)) {
                ...
                $require_verification = true; // semua zon perlu disahkan
                $is_quota_full = true;
                ...
                foreach ($vessels as $vessel) {
                    ...
                    if (!$vessel->isQuotaFull() && !$require_verification) {
    ...
    public function update(Request $request, string $id)
    {
        ...
        $email_rule = Rule::unique('profile_users')->where('user_type', $type_vessel_manager)->ignore($profile->id);
        if ($request->select_manager) {
            $email_rule->ignore($request->select_manager);
        }
