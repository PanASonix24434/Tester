...
class sokonganUlasanController extends Controller
{
    ...
    public function index()
    {
        ...
        $positiveFeedback = ['804'];
        $negativeFeedback = ['805-1'];
        ...
        return view('app.NelayanDarat.kadPendaftaran.pegawai.5_sokonganUlasan.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        return view('app.NelayanDarat.kadPendaftaran.pegawai.5_sokonganUlasan.create', compact(
            'application',
            'roleName',
            'applicationType',
            'moduleName',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            $statusCode = $request->review_flag ? '805' : '804-1';
            $statusId = CodeMaster::where('type', 'application_status')
                ->where('code', $statusCode)
                ->value('id');
            ...
            return redirect()->route('kadPendaftaran.sokonganUlasan-08.index')
                ->with('success', 'Berjaya disimpan.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Ralat: ' . $e->getMessage());
