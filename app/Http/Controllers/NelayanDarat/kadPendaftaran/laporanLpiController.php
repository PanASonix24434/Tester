...
class laporanLpiController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('app.NelayanDarat.kadPendaftaran.pegawai.3_laporanLpi.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        $inspection = darat_vessel_inspection::where('application_id', $application->id)->where('user_id', $user->id)->first();
        ...
        $mainInspectionEquipment = darat_inspection_equipment::where('application_id', $application->id)->where('type', 'UTAMA')->first();

        $additionalInspectionEquipment = darat_inspection_equipment::where('application_id', $application->id)->where('type', 'TAMBAHAN')->get();

        //    dd($mainInspectionEquipment);

        // ===== [8] Return to View =====
        return view('app.NelayanDarat.kadPendaftaran.pegawai.3_laporanLpi.create', compact(
            'application',
            'user',
            'roleName',
    ...
    public function storePeralatanKeselamatan(Request $request, $id)
    {
        ...
        try {
            ...
            $test = darat_vessel_inspection::updateOrCreate(
                ['application_id' => $application->id,  'user_id'                   => $user->id,],
                [
        ...
        public function storePeralatanMenangkapIkan(Request $request, $id)
    {
        ...
        try {
            ...
            // MAIN equipment - Only one item
            $main = $request->input('main');
            if (!empty($main['name']) && !empty($main['quantity'])) {
                $filePath = null;
                ...
                darat_inspection_equipment::updateOrCreate(
                    [
                        'application_id' => $application->id,
    ...
    public function storePengesahanPemeriksaan(Request $request, $id)
    {
        ...
        try {
            ...
            $request->validate([
                'keadaan_vesel'              => 'nullable   |string|max:255',
    ...
    public function store($id)
    {
        ...
        try {
            ...
            return redirect()->route('kadPendaftaran.laporanLpi-08.index')
    ...
    public function viewInspection(Request $request)
    {
        ...
        if (empty($id) || empty($field)) {
            abort(400, 'ID dan medan fail diperlukan.');
        }
    ...
    public function viewEquipment(Request $request)
    {
        ...
        if (empty($id) || empty($type)) {
            abort(400, 'Parameter ID dan jenis peralatan diperlukan.');
        }

        $equipment = darat_inspection_equipment::findOrFail($id);

        if ($equipment->type !== $type) {
            abort(403, 'Jenis peralatan tidak sah.');
        }
