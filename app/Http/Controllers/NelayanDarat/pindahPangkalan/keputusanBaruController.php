...
class keputusanBaruController extends Controller
{
    ...
    public function index()
    {
        ...
        // Get all applications with matching status & entity_id
        $applications = darat_application::with(['applicationType', 'applicationStatus', 'user', 'fetchPin'])
            ...
            ->where('new_entity_id', $userEntityId)
        ...
        return view('app.NelayanDarat.pindahPangkalan.pegawai.11_keputusan.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        return view('app.NelayanDarat.pindahPangkalan.pegawai.11_keputusan.create', compact(
            'application',
            'roleName',
            'applicationType',
            'moduleName',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            $formData = json_decode($temp->form_data ?? '{}', true);
            $jettyInfo = $formData['tab3_jetty_info'] ?? [];

            $jenisPindah = $jettyInfo['jenisPindah'] ?? null;
            // Get review/decision flags

            $reviewFlag   = $request->input('review_flag');
            $decisionFlag = $request->input('decision_flag');

            // Default status code logic
            ...
            // // ✅ If both review & decision are '1' AND pindah is antara → override to 308
            // if ($reviewFlag === '1' && $decisionFlag === '1' && $jenisPindah === 'antara') {
            //     $statusCode = 308;
            // }
            ...
            if ($reviewFlag === '1') {
                $resultMessage = match ($statusCode) {
                    317 => 'PERMOHONAN ANDA TELAH DI LULUSKAN.',
                    // 308 => 'PERMOHONAN ANDA TELAH DI LULUSKAN. ',
                    316 => 'PERMOHONAN TIDAK LULUS, SILA LAKUKAN RAYUAN.',
                ...
                //     ];
                // } else {
                //     // Normal approval/rejection → include pin (if applicable)
                //     $resultDetails = [
                //         'no_rujukan'       => $application->no_rujukan,
                //         'applicant_icno'   => $userDetails->identity_card_number ?? $user->username,
                //         'application_type' => $application->applicationType->name ?? 'Tidak Diketahui',
            ...
            return redirect()->route('pindahPangkalan.keputusan-03.index')
