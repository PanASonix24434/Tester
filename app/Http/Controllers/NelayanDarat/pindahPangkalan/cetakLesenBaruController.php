...
class cetakLesenBaruController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('app.NelayanDarat.pindahPangkalan.pegawai.14_cetakLesenBaru.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        return view('app.NelayanDarat.pindahPangkalan.pegawai.14_cetakLesenBaru.create', compact(
            'application',
            'roleName',
            'applicationType',
            'moduleName',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            if ($jettyInfo) {
                // Step 1: Save or update main base record
                $jettyBase = darat_base_jetties::updateOrCreate(
                    [
                        'user_id' => $user->id,
                ...
                // Step 2: Optional - mark previous histories as inactive
                darat_base_jetty_history::where('user_id', $user->id)
                    ->update(['is_active' => false]);

                // Step 3: Create new history record using only ID values
                darat_base_jetty_history::create([
                    'user_id'     => $jettyBase->user_id,
                    'jetty_base_id'     => $jettyBase->id,
                    'state_id'    => $jettyBase->state_id,
                    'district_id' => $jettyBase->district_id,
                    'jetty_id'    => $jettyBase->jetty_id,
                    'river_id'    => $jettyBase->river_id,
                    'is_active'   => true,
                    'created_by'  => auth()->id(),
                    'updated_by'  => auth()->id(),
                ]);
            ...
            return redirect()->route('pindahPangkalan.cetakLesen-03.index')
