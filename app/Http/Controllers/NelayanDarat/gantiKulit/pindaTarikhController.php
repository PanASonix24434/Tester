...
class pindaTarikhController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('app.NelayanDarat.gantiKulit.pegawai.2_pindaTarikh.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        return view('app.NelayanDarat.gantiKulit.pegawai.2_pindaTarikh.create', compact(
            'application',
            'roleName',
            'applicationType',
            'moduleName',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            // $formData = json_decode($temp->form_data ?? '{}', true);
            // $disposal = $formData['tab7_disposal'] ?? [];
            // $jenisPermohonan = $disposal['jenisPermohonan'] ?? null;

            // // Determine status code
            // if ($request->review_flag == '1') {
            //     if ($jenisPermohonan === 'lupus') {
            //         $statusCode = 615;
            //     } elseif ($jenisPermohonan === 'gantikulit') {
            //         $statusCode = 604;
            //     } else {
            //         throw new \Exception('Maklumat jenis permohonan tidak lengkap.');
            //     }
            // } else {
            //     $statusCode = 302; // Tidak Lengkap
            // }
            ...
            return redirect()->route('gantiKulit.pindaLpi-06.index')
