...
class semakanDokumenController extends Controller
{
    ...
    public function index()
    {
        ...
        $positiveFeedback = ['901'];
        $negativeFeedback = ['904-1'];
        ...
        return view('app.NelayanDarat.baharuKadNelayan.pegawai.1_semakanDokumen.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        return view('app.NelayanDarat.baharuKadNelayan.pegawai.1_semakanDokumen.create', compact(
            'application',
            'roleName',
            'applicationType',
            'moduleName',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            if ($inspectionOff && $inspectionOff->valid_date) {
                $isStillValid = Carbon::parse($inspectionOff->valid_date)->toDateString() >= Carbon::now()->toDateString();
            }

            // Status code logic
            if ($request->review_flag == '1') {
                // Lengkap
                $statusCode = $isStillValid ? '904' : '902';
            } else {
                // Tidak Lengkap
                $statusCode = '901-1';
            }
            ...
            return redirect()->route('baharuKadNelayan.semakanDokumen-09.index')
