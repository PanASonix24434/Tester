...
class permohonanController extends Controller
{
    ...
    public function index()
    {
        ...
        $negativeFeedback = ['901-1'];

        $draftFeedback = ['0'];

        $appealFeedback = ['911-2', '904-2']; // Get the results
        ...
        return view('app.NelayanDarat.baharuKadNelayan.pemohon.index', compact('applications', 'draftFeedback', 'negativeFeedback', 'appealFeedback', 'applicationType', 'moduleName', 'roleName'));
    }
    ...
    public function create()
    {
        ...
        return view('app.NelayanDarat.baharuKadNelayan.pemohon.create', compact(
            'user',
            'roleName',
            'applicationType',
            'applicationTypeId',
    ...
    public function negativeFeedback($id)
    {
        ...
        return view('app.NelayanDarat.baharuKadNelayan.pemohon.incomplete', compact(
            'user',
            'applicationType',
            'application',
            'moduleName',
    ...
    public function appealFeedback($id)
    {
        ...
        $applicationLogs = darat_application_log::with(['applicationStatus', 'creator'])
            ...
            ->whereHas('applicationStatus', function ($query) {
                $query->where('code', '911-2');
            })
            ->first();

        $userDetail = ProfileUsers::where('user_id', $user->id)->where('is_active', 1)->first();

        return view('app.NelayanDarat.baharuKadNelayan.pemohon.appeal', compact(
            'application',
            'user',
            'roleName',
            'segment1',
   ...
   public function storeAppeal(Request $request)
    {
        ...
        try {
            ...
            $statusId = CodeMaster::where('type', 'application_status')->where('code', '908')->value('id');
            ...
            return redirect()->route('baharuKadNelayan.permohonan-09.index')->with('success', 'Rayuan berjaya dihantar.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->filled('user_statement')) {
                ...
                DB::commit();
                return redirect()->route('baharuKadNelayan.permohonan-09.index')
