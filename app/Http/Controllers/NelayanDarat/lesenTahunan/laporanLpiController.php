...
class laporanLpiController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('app.NelayanDarat.lesenTahunan.pegawai.3_laporanLpi.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        $inspectionEquipment = darat_inspection_equipment::where('inspection_id', $inspectionOff->id)->get();

        $mainInspectionEquipment = darat_inspection_equipment::where('inspection_id', $inspectionOff->id)->where('type', 'UTAMA')->get();

        $additionalInspectionEquipment = darat_inspection_equipment::where('inspection_id', $inspectionOff->id)->where('type', 'TAMBAHAN')->get();


        // ===== [8] Return to View =====
        return view('app.NelayanDarat.lesenTahunan.pegawai.3_laporanLpi.create', compact(
            'application',
            'user',
            'roleName',
    ...
    public function store($id)
    {
        ...
        try {
            ...
            return redirect()->route('lesenTahunan.laporanLpi-02.index')
    ...
    public function viewEquipmentFile(Request $request)
    {
        ...
        $applicaitonID = $request->query('application');

        $type = $request->query('type');
        $index = $request->query('index');

        $temp = darat_application_temp::where('application_id', $applicaitonID)
            ->where('status', 'pending')
            ->latest()
            ->first();
