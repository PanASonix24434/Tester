...
class semakanDokumenController extends Controller
{
    ...
    public function index()
    {
        ...
        $positiveFeedback = ['201'];
        $negativeFeedback = ['204-1'];
        ...
        return view('app.NelayanDarat.lesenTahunan.pegawai.1_semakanDokumen.index', compact(
            'applications',
            'positiveFeedback',
            'negativeFeedback',
            'applicationType',
    ...
    public function create($id)
    {
        ...
        return view('app.NelayanDarat.lesenTahunan.pegawai.1_semakanDokumen.create', compact(
            'application',
            'roleName',
            'applicationType',
            'moduleName',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            // Determine if inspection is still valid
            $vesselOff = darat_vessel::where('user_id', $user->id)
                ->where('is_active', 1)
                ->first();
            ...
            if ($request->review_flag == '1') {
                // Lengkap
                $statusCode = $isStillValid ? '204' : '202';
            } else {
                // Tidak Lengkap
                $statusCode = '201-1';
            }
            ...
            return redirect()->route('lesenTahunan.semakanDokumen-02.index')
