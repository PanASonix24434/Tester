...
class SokonganNegeriController extends Controller
{
    /**
    ...
    {
        $request->user()->isAuthorize('sokongannegeri');
        $user = $request->user();

        $helper = new Helper();
        $statusId = $helper->getCodeMasterIdByTypeName('kru_application_status','DISEMAK NEGERI');
        ...
        ->where(function ($query) use ($appTypeId, $statusId, $wilayahIds, $user) {
        ...
        })->orWhere(function ($query) use ($appTypeId,$statusId,$wilayahIds, $user) {
            $query->where('kru_application_type_id', $appTypeId) //kru01,kru02,kru03,kru04
            ->where('kru_applications.kru_application_status_id',$statusId)
        ...
        return view('app.kru.kru03.sokongannegeri.index', [
    ...
    public function show(string $id)
    {
        ...
        return view('app.kru.kru03.sokongannegeri.show', [
            'id' => $id,
            'app' => $app,
            'vesel' => $vesel,
    ...
    public function update(Request $request, string $id)
    {
        if ($request->action=='save'){
            ...
            try {
                ...
                $auditLog->log('kru03SokonganNegeri', 'update', $audit_details);
                DB::commit();
            }
            catch (Exception $e) {
                DB::rollback();
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru03SokonganNegeri', 'update', $audit_details, $e->getMessage());
            ...
            try {
                ...
                if($request->applicationStatus == 'supported'){
                    $status_id = $helper->getCodeMasterIdByTypeName('kru_application_status','DISOKONG NEGERI');
                    $status_id2 = $status_id;
                }
                else if($request->applicationStatus == 'notsupported'){
                    $status_id = $helper->getCodeMasterIdByTypeName('kru_application_status','TIDAK DISOKONG NEGERI');
                    $status_id2 = $status_id;
                }
                    ...
                    $appEntity = Entity::find(KruApplication::find($id)->entity_id);
                    if($appEntity->parent_id == $sarawakEntityId){
            ...
            return redirect()->route('gantiankadpendaftarannelayan.sokongannegeri.index')->with('alert', 'Maklumat berjaya dihantar !!');
        }
    }
