...
class SemakanDaerahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('app.kru.kru01.semakandaerah.index', [
    ...
    public function show(string $id)
    {
        ...
        $docSiasatan = KruDocument::where('kru_application_kru_id',$kru->id)->where('description',KruDocument::DOC_SIASATAN)->latest()->first();
        ...
        return view('app.kru.kru01.semakandaerah.show', [
            'id' => $id,
            'app' => $app,
            'kru' => $kru,
            'applicant' => $applicant,
            'docs' => $docs,
            'docSiasatan' => $docSiasatan,
            //------------start logs----------------
            'logs' => $logs,
    ...
    public function update(Request $request, string $id)
    {
        if ($request->action=='save'){
            ...
            try {
                ...
                if ($request->file('docSiasatan')) {
                    $file = $request->file('docSiasatan');
                    $file_replace = str_replace(' ', '', $file->getClientOriginalName());
                    $filename = $file_replace;
                    $path = $request->file('docSiasatan')->store('public/kru/krudoc');

                    $doc = new KruDocument();
                    $doc->kru_application_kru_id = $appKru->id;
                    $doc->file_name = $filename;
                    $doc->file_path = $path;
                    $doc->description = KruDocument::DOC_SIASATAN;
                ...
                $auditLog->log('kru01SemakanDaerah', 'update', $audit_details);
                DB::commit();
            }
            catch (Exception $e) {
                DB::rollback();
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru01SemakanDaerah', 'update', $audit_details, $e->getMessage());
            ...
            return redirect()->route('kadpendaftarannelayan.semakandaerah.index')->with('alert', 'Maklumat berjaya dihantar !!');
        }
