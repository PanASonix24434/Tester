...
class SokonganDaerahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        ->where(function ($query) use ($appTypeId,$statusId,$user) {
            $query->where('kru_applications.application_type','KRU BARU')
        ...
        })->orWhere(function ($query) use ($appTypeId,$statusId,$user,$sarawakEntityId) {
            $query->where('kru_applications.application_type','GANTI KRU')
        ...
        return view('app.kru.kru01.sokongandaerah.index', [
    ...
    public function show(string $id)
    {
        ...
        return view('app.kru.kru01.sokongandaerah.show', [
            'id' => $id,
            'app' => $app,
            'kru' => $kru,
            'applicant' => $applicant,
            ...
            //-------------end logs----------------
            'savedLog' => $savedLog,
            'kruHealths' => $helper->getCodeMastersByTypeOrder('kru_health'),
        ]);
    }
    ...
    public function update(Request $request, string $id)
    {
        if ($request->action=='save'){
            ...
            try {
                if($request->applicationStatus == 'supported' || $request->applicationStatus == 'notsupported'){
                    $app2 = KruApplicationKru::where('kru_application_id',$id)->first();
                    $app2->kru_health_id = $request->kruHealth;
                    $app2->updated_by = $request->user()->id;
                    $app2->save();
                }
                ...
                $auditLog->log('kru01SokonganDaerah', 'update', $audit_details);
                DB::commit();
            }
            catch (Exception $e) {
                DB::rollback();
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru01SokonganDaerah', 'update', $audit_details, $e->getMessage());
            ...
            return redirect()->route('kadpendaftarannelayan.sokongandaerah.index')->with('alert', 'Maklumat berjaya dihantar !!');
        }
    }
