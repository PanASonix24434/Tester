...
class TerimaanBayaranController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $statusId3 = $helper->getCodeMasterIdByTypeName('kru_application_status','SSD LAMA DITERIMA');
        ...
        ->where(function ($query) use ($appTypeId, $statusId, $statusId2, $user ) {
            $query->where('kru_application_type_id', $appTypeId)
                ->where('kru_applications.application_type','KRU BARU')
                ->whereIn('kru_applications.kru_application_status_id',[$statusId,$statusId2]);
                if($user->username != '111111111111'){
                    $query->where('kru_applications.entity_id',$user->entity_id);
                }
        })->orWhere(function ($query) use ($appTypeId, $statusId, $statusId2, $user) {
            $query->where('kru_application_type_id', $appTypeId)
                ->where('kru_applications.application_type','GANTI KRU')
                ->whereIn('kru_applications.kru_application_status_id',[$statusId,$statusId2]);
                if($user->username != '111111111111'){
                    $query->where('kru_applications.entity_id',$user->entity_id);
                }
        ...
        return view('app.kru.kru01.terimaanbayaran.index', [
    ...
    public function storeReceipt(Request $request,string $id)
    {
        ...
        if($items==null){
            ...
            try {
                ...
                $auditLog->log('kru01TerimaanBayaran', 'create', $audit_details);
                DB::commit();
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru01TerimaanBayaran', 'create', $audit_details, $e->getMessage());

                return redirect()->back()->with('alert', 'Maklumat Bayaran gagal disimpan !!');
            }

        }
        return redirect()->route('kadpendaftarannelayan.terimaanbayaran.show', $id)->with('alert', 'Maklumat Bayaran berjaya disimpan !!');
    }
    ...
    public function show(string $id)
    {
        ...
        return view('app.kru.kru01.terimaanbayaran.show', [
            'id' => $id,
            'app' => $app,
            'kru' => $kru,
            'applicant' => $applicant,
    ...
    public function createReceipt(string $id)
    {
        ...
        return view('app.kru.kru01.terimaanbayaran.create', [
            'id' => $id,
            'app' => $app,
            // 'app2' => $app2,
            // 'vesel' => $vesel,
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            $auditLog = new AuditLog();
            $auditLog->log('kru01TerimaanBayaran', 'update', $audit_details);
            DB::commit();
        }
        catch (Exception $e) {
            DB::rollback();
            ...
            $auditLog = new AuditLog();
            $auditLog->log('kru01TerimaanBayaran', 'update', $audit_details, $e->getMessage());

            return redirect()->back()->with('alert', 'Maklumat Bayaran gagal dihantar !!');
        }
        return redirect()->route('kadpendaftarannelayan.terimaanbayaran.index')->with('alert', 'Maklumat Bayaran berjaya dihantar !!');
    }
