...
use App\Mail\Kru\Kru02Approved;
...
class KeputusanDaerahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        ->select('kru_applications.id','reference_number','vessel_id','users.name','kru_applications.entity_id','kru_application_status_id','kru_applications.submitted_at')->sortable();

        return view('app.kru.kru02.keputusandaerah.index', [
    ...
    public function show(string $id)
    {
        ...
        return view('app.kru.kru02.keputusandaerah.show', [
            'id' => $id,
            'app' => $app,
            'vessel' => $vessel,
    ...
    public function showKru(string $id)
    {
        ...
        return view('app.kru.kru02.keputusandaerah.showKru', [
    ...
    public function update(Request $request, string $id)
    {
        if ($request->action=='save'){
            ...
            try {
                ...
                if($request->applicationStatus == 'approved'){
                    ...
                    foreach ($selKrus as $selKru) {
                        ...
                        if(($nelayanMarin != null) && ($nelayanMarin->vessel_id == $app->vessel_id)){
                            $nelayanMarin->address1 = $selKru->address1;
                            $nelayanMarin->address2 = $selKru->address2;
                            $nelayanMarin->address3 = $selKru->address3;
                            $nelayanMarin->postcode = $selKru->postcode;
                            $nelayanMarin->city = $selKru->city;
                            $nelayanMarin->district_id = $selKru->district_id;
                            $nelayanMarin->state_id = $selKru->state_id;
                            $nelayanMarin->home_contact_number = $selKru->home_contact_number;
                            $nelayanMarin->mobile_contact_number = $selKru->mobile_contact_number;
                            $nelayanMarin->email = $selKru->email;
        
                            $nelayanMarin->registration_start = $app->registration_start;
                            $nelayanMarin->registration_end = $app->registration_end;
                            $nelayanMarin->kru_application_kru_id = $selKru->id;
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru02KeputusanDaerah', 'update', $audit_details);
                DB::commit();
                ...
                if($request->applicationStatus == 'approved'){
                    ...
                    Mail::to($pemilikVesel->email)->queue(new Kru02Approved($mailDataArr));
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru02KeputusanDaerah', 'update', $audit_details, $e->getMessage());
    
                return redirect()->back()->with('alert', 'Maklumat gagal dihantar !!');
            }
            return redirect()->route('pembaharuankadpendaftarannelayan.keputusandaerah.index')->with('alert', 'Maklumat berjaya dihantar !!');
        }
    }
