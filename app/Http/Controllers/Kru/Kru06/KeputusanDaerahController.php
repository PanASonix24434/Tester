...
use App\Mail\Kru\Kru06Approved;
...
class KeputusanDaerahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('app.kru.kru06.keputusandaerah.index', [
    ...
    public function show(string $id)
    {
        ...
        return view('app.kru.kru06.keputusandaerah.show', [
            'id' => $id,
            'app' => $app,
    ...
    public function update(Request $request, string $id)
    {
        if ($request->action=='save'){
            ...
            try {
                ...
                $auditLog->log('kru06KeputusanDaerah', 'update', $audit_details);
                DB::commit();
            }
            catch (Exception $e) {
                DB::rollback();
                ...
                $auditLog = new AuditLog();
                $auditLog->log('kru06KeputusanDaerah', 'update', $audit_details, $e->getMessage());
            ...
            try {
                ...
                if($request->applicationStatus == 'approved'){
                    ...
                    $appKrus = KruApplicationForeignKru::where('kru_application_id',$id)->where('has_plks',true)->where('approved',true)->get();
                    foreach ($appKrus as $appKru) {
                        $appKru->selected_for_approval = true;
                        $appKru->updated_by = Auth::id();
                        $appKru->save();

                        $foreignCrew = ForeignCrew::where('passport_number',$appKru->passport_number)->first();
                        if($foreignCrew==null){
                            $foreignCrew = new ForeignCrew();
                            $foreignCrew->created_by = Auth::id();
                            $foreignCrew->passport_number = $appKru->passport_number;
                        }
                        $foreignCrew->name = $appKru->name;
                        $foreignCrew->birth_date = $appKru->birth_date;
                        $foreignCrew->passport_end_date = $appKru->passport_end_date;
                        $foreignCrew->gender_id = $appKru->gender_id;
                        $foreignCrew->source_country_id = $appKru->source_country_id;
                        $foreignCrew->crew_whereabout = $appKru->crew_whereabout;
                        $foreignCrew->foreign_kru_position_id = $appKru->foreign_kru_position_id;
                        $foreignCrew->plks_end_date = $appForeign->plks_end_date;
                        $foreignCrew->plks_number = $appKru->plks_number;
                        
                        $foreignCrew->vessel_id = $vessel->id;
                        $foreignCrew->kru_application_foreign_kru_id = $appKru->id;
                        $foreignCrew->save();
                    }
                ...
                if($request->applicationStatus == 'approved'){
                    ...
                    $selKrus = KruApplicationForeignKru::where('kru_application_id',$id)->where('selected_for_approval',true)->select('passport_number','name')->get()->toArray();
                    $mailDataArr = array(
                    ...
                    Mail::to($pemilikVesel->email)->queue(new Kru06Approved($mailDataArr));
            ...
            return redirect()->route('kelulusanpenggunaankrubukanwarganegara.keputusandaerah.index')->with('alert', 'Maklumat berjaya dihantar !!');
        }
    }
