...
class CodeMaster extends Model
{
    ...
    {
        $model = CodeMaster::where('type', $type)->where('name', $name)->first();
    ...
    public static function seed($type, $code, $name, $name_ms, $order = null, $parent = null, $parent_name = null)
    {
        ...
        if (!CodeMaster::seedExists($type, $name, $code, $parent_id)) {
            ...
            if ($adminUser instanceof \Illuminate\Database\Eloquent\Builder) {
                $adminUser = $adminUser->first();
            }
            $code_master->created_by = $adminUser ? $adminUser->id : null;
            $code_master->updated_by = $adminUser ? $adminUser->id : null;
            $code_master->save();
        }
    }
...
// Remove duplicate import and keep only the controller definition

class SomeController extends Controller
{
    public function show() {
        // Fetching all active CodeMasters of a certain type (e.g., 'races')
        $codeMasters = CodeMaster::where('type', 'races')->get();

        return view('someview', compact('codeMasters'));
    }
}
