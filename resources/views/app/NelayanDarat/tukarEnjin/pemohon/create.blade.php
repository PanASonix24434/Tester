                ...
                <div class="card card-primary card-tabs">
                    ...
                    <div class="card-body">
                        ...
                        <div class="tab-content" id="pills-tabContent">
                                ...
                                <section class="mb-4">
                                    ...
                                    <form id="store_tab5" method="POST" enctype="multipart/form-data"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <small>Sila lengkapkan semua medan yang disediakan dan bertanda (<span
                                                    class="text-danger">*</span>) sebagai wajib diisi.</small>
                                        </div>
                                        ...
                                        <div class="  rounded border bg-light   p-2">
                                            ...
                                            <div class="row">
                                                <div class="col-md-4 mb-3">
                                                    <label class="form-label">Jenama Enjin <span
                                                            class="text-danger">*</span></label>
                                                    <select name="new_brand" class="form-select" required>
                                                        <option value="">Sila Pilih Jenama</option>
                                                        @foreach($engineBrandList as $id => $name)
                                                        <option value="{{ $name }}" {{ old('new_brand',
                                                            $vesselTemp['new_brand'] ?? '' )==$name ? 'selected' : ''
                                                            }}>
                                                            {{ $name }}
                                                        </option>
                                                        @endforeach
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <label class="form-label">Model Enjin <span
                                                            class="text-danger">*</span></label>
                                                    <input type="text" name="new_model" class="form-control"
                                                        placeholder="Contoh: 15FMH"
                                                        value="{{ old('new_model', $vesselTemp['new_model'] ?? '') }}"
                                                        required>
                                                </div>
                                                <div class="col-md-4 mb-3">
                                                    <label class="form-label">Kuasa Kuda (KK) <span
                                                            class="text-danger">*</span></label>
                                                    <input type="number" name="new_horsepower" class="form-control"
                                                        placeholder="Contoh: 30"
                                                        value="{{ old('new_horsepower', $vesselTemp['new_horsepower'] ?? '') }}"
                                                        required>
                                                </div>
                                            </div>
                         ...
                         @php
    $engineDoc = collect($documentsTemp)->firstWhere('title', 'Gambar Enjin');
    $receiptDoc = collect($documentsTemp)->firstWhere('title', 'Resit Pembayaran Enjin');
@endphp
...
<div class="tab-pane fade" id="content-tab6" role="tabpanel" aria-labelledby="tab6-link">
    <form method="POST" id="store_tab6"
        ...
        <section>
            <div class="mb-3">
                ...
                <small class="text-muted d-block mb-3">Sila muat naik gambar enjin dan resit pembelian enjin.</small>
                <hr>
            </div>

            {{-- Gambar Enjin --}}
            <div class="mb-4">
                <label for="engine_picture" class="form-label">Gambar Enjin <span class="text-danger">*</span></label>
                <div class="d-flex align-items-center gap-2">
                    <div style="flex-grow: 1;">
                        <div class="custom-file">
                            ...
                            <label class="custom-file-label" for="engine_picture">
                                {{ isset($engineDoc['file_path']) ? basename($engineDoc['file_path']) : 'Pilih Fail' }}
                            </label>
                        </div>
                    </div>
                    ...
                    @if (!empty($engineDoc['file_path']))
                        <a class="btn btn-primary"
                           href="{{ route('tukarEnjin.permohonan-05.viewDocument', ['type' => 'required', 'index' => array_search($engineDoc, $documentsTemp)]) }}"
                           target="_blank">
            ...
            {{-- Resit Pembayaran Enjin --}}
            <div class="mb-4">
                <label for="payment_receipt" class="form-label">Resit Pembelian Enjin <span class="text-danger">*</span></label>
                <div class="d-flex align-items-center gap-2">
                    <div style="flex-grow: 1;">
                        <div class="custom-file">
                            <input type="file" class="custom-file-input" id="payment_receipt" name="payment_receipt"
                                   accept=".jpg,.jpeg,.png,.pdf" required>
                            <label class="custom-file-label" for="payment_receipt">
                                {{ isset($receiptDoc['file_path']) ? basename($receiptDoc['file_path']) : 'Pilih Fail' }}
                            </label>
                        </div>
                    </div>

                    @if (!empty($receiptDoc['file_path']))
                        <a class="btn btn-primary"
                           href="{{ route('tukarEnjin.permohonan-05.viewDocument', ['type' => 'required', 'index' => array_search($receiptDoc, $documentsTemp)]) }}"
                           target="_blank">
                            <i class="fa fa-search p-1"></i>
                        </a>
                    @endif
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ['engine_picture', 'payment_receipt'].forEach(function (id) {
    ...
    @endpush
                    ...
                    <script>
                        let currentTab = 1;
    const totalTabs = 7;
    const tabsWithSaveRequirement = [5, 6]; // Tabs that require "Simpan" before proceeding

    document.addEventListener("DOMContentLoaded", () => {
        const lastTab = parseInt(sessionStorage.getItem("lastTab"));
        if (lastTab && lastTab >= 1 && lastTab <= totalTabs) {
            currentTab = lastTab;
        }
        ...
        document.getElementById("nextTabBtn").onclick = () => handleNext();
        document.getElementById("backTabBtn").onclick = () => handleBack();

        // Setup "Simpan" buttons for tab 5 & 6
        [5, 6].forEach(tabNum => {
            ...
            if (btn) {
                btn.onclick = () => {
                    ...
                    updateButtons(); // Refresh Simpan visibility
                };
            }
        });
    });
    ...
    function updateButtons() {
        ...
        // Show simpan only on current tab if it's 5 or 6
        for (let i = 1; i <= totalTabs; i++) {
            const simpanBtn = document.getElementById(`saveBtn${i}`);
            if (simpanBtn) {
                simpanBtn.style.display = (tabsWithSaveRequirement.includes(i) && currentTab === i)
                    ? 'inline-block' : 'none';
            }
        }
    }
