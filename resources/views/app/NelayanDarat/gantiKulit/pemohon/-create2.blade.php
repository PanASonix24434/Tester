...
<div id="app-content">
    ...
    @php
        $vesselOldIndex = collect($documents)->search(fn($doc) => $doc['title'] === 'Gambar Vesel Asal');
        $vesselOld = $vesselOldIndex !== false ? $documents[$vesselOldIndex] : null;

        $vesselNewIndex = collect($documents)->search(fn($doc) => $doc['title'] === 'Gambar Vesel Baharu');
        $vesselNew = $vesselNewIndex !== false ? $documents[$vesselNewIndex] : null;
    @endphp

    <h4 class="fw-bold mb-2">Muat Naik Dokumen</h4>
    <small class="text-muted">Sila muat naik dokumen sokongan yang berkaitan bagi tujuan semakan dan pengesahan permohonan.</small>
    <hr>

    {{-- Gambar Vesel Asal --}}
    <div class="mb-3">
        <label for="vesselOld" class="form-label fw-bold">Gambar Vesel Asal <span class="text-danger">*</span></label>
        <div class="d-flex gap-2">
            <input class="form-control" type="file" name="vesselOld" id="vesselOld" required>
            @if ($vesselOld)
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#gambarVeselAsal">
                    <i class="fa fa-search"></i>
                </button>
            @endif
        </div>
    ...
    {{-- Modal for Gambar Vesel Asal --}}
    @if ($vesselOld)
        <div class="modal fade" id="gambarVeselAsal" tabindex="-1" aria-labelledby="gambarVeselAsalLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="gambarVeselAsalLabel">Lampiran: Gambar Vesel Asal</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                    </div>
                    <div class="modal-body" style="height: 80vh;">
                        <iframe src="{{ route('gantiKulit.permohonan-06.viewDocument', ['index' => $vesselOldIndex]) }}"
                                style="width:100%; height:100%; border:none;"></iframe>
                    </div>
                </div>
            </div>
    ...
    {{-- Gambar Vesel Baru --}}
    <div class="mb-3">
        <label for="vesselNew" class="form-label fw-bold">Gambar Vesel Baru <span class="text-danger">*</span></label>
        <div class="d-flex gap-2">
            <input class="form-control" type="file" name="vesselNew" id="vesselNew" required>
            @if ($vesselNew)
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#gambarVeselBaru">
                    <i class="fa fa-search"></i>
                </button>
            @endif
        </div>
    ...
    {{-- Modal for Gambar Vesel Baru --}}
    @if ($vesselNew)
        <div class="modal fade" id="gambarVeselBaru" tabindex="-1" aria-labelledby="gambarVeselBaruLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="gambarVeselBaruLabel">Lampiran: Gambar Vesel Baru</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                    </div>
                    <div class="modal-body" style="height: 80vh;">
                        <iframe src="{{ route('gantiKulit.permohonan-06.viewDocument', ['index' => $vesselNewIndex]) }}"
                                style="width:100%; height:100%; border:none;"></iframe>
                    </div>
                </div>
            </div>
