...
<div id="app-content">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <input type="text" class="form-control" id="district" name="district"
                                                value="{{ old('state', $userDetail->secondary_district_name ?? '') }}"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <input type="text" class="form-control" id="secondary_district"
                                                ...
                                                value="{{ old('state', $userDetail->district_name ?? '') }}"
                                                placeholder="Masukkan Daerah" readonly>
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <label for="secondary_state  " class="form-label">Negeri</label>
                                            <input type="text" class="form-control" id="state" name="state"
                                                value="{{ old('state', $userDetail->state_name ?? '') }}"
                                                placeholder="Masukkan Negeri" readonly>
    ...
    <!-- Menerima Pencen -->
    <div class="mb-3">
        <label for="receive_pension" class="form-label">Menerima Pencen<span class="text-danger">*</span></label>
        <select class="form-select" id="receive_pension" name="receive_pension" required>
            <option value="">Sila Pilih </option>
            <option value="1" {{ old('receive_pension', $fishermanData['receive_pension'] ?? '') == 1 ? 'selected' : '' }}>YA</option>
            <option value="0" {{ old('receive_pension', $fishermanData['receive_pension'] ?? '') == 0 ? 'selected' : '' }}>TIDAK</option>
        </select>
    </div>

    <!-- Pencarum KWSP -->
    <div class="mb-3">
        <label for="epf_contributor" class="form-label">Pencarum KWSP <span class="text-danger">*</span></label>
        <select class="form-select" id="epf_contributor" name="epf_contributor" required>
            <option value="">Sila Pilih </option>
            <option value="1" {{ old('epf_contributor', $fishermanData['epf_contributor'] ?? '') == 1 ? 'selected' : '' }}>YA</option>
            <option value="0" {{ old('epf_contributor', $fishermanData['epf_contributor'] ?? '') == 0 ? 'selected' : '' }}>TIDAK</option>
        </select>
    </div>

    <!-- Jenis KWSP -->
    <div id="epf_type_container" class="mb-3" style="display: none;">
    ...
    <div class="mb-3">
        <label for="receive_financial_aid" class="form-label">Menerima Bantuan Kewangan<span class="text-danger">*</span></label>
        <select class="form-select" id="receive_financial_aid" name="receive_financial_aid" required>
            <option value="">Sila Pilih </option>
            <option value="1" {{ old('receive_financial_aid', $fishermanData['receive_financial_aid'] ?? '') == 1 ? 'selected' : '' }}>YA</option>
            <option value="0" {{ old('receive_financial_aid', $fishermanData['receive_financial_aid'] ?? '') == 0 ? 'selected' : '' }}>TIDAK</option>
        </select>
    </div>

    <!-- Agensi Bantuan -->
    <div id="financial_aid_agency_div" style="display: none;">
        <label class="form-label">Agensi Memberi Bantuan Kewangan <span class="text-danger">*</span></label>
        <div id="financial_aid_agency_container">
            ...
            @forelse ($agencies as $agency)
                <div class="input-group agency-row mb-2">
                    <input type="text" name="financial_aid_agency[]" class="form-control" value="{{ $agency }}" placeholder="Masukkan Nama Agensi Penyedia Bantuan Kewangan">
            ...
            @empty
                <div class="input-group agency-row mb-2">
                    <input type="text" name="financial_aid_agency[]" class="form-control" placeholder="Masukkan Nama Agensi Penyedia Bantuan Kewangan">
                    <button type="button" class="btn btn-danger remove-agency" title="Buang">
                        <i class="fa fa-trash"></i>
                    </button>
                </div>
        ...
        <button type="button" class="btn btn-primary mt-2" id="add-agency-btn">+ Tambah Agensi</button>
    </div>
</div>
                                </form>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const financialAid = document.getElementById('receive_financial_aid');
    const aidAgencyDiv = document.getElementById('financial_aid_agency_div');
    ...
    function toggleEpfType() {
        epfTypeContainer.style.display = epfContributor.value === '1' ? 'block' : 'none';
        if (epfContributor.value !== '1') {
            document.getElementById('epf_type').value = '';
        }
    }

    function toggleAidAgency() {
        aidAgencyDiv.style.display = financialAid.value === '1' ? 'block' : 'none';
        if (financialAid.value !== '1') {
            const inputs = aidAgencyDiv.querySelectorAll('input[name="financial_aid_agency[]"]');
    ...
    addBtn.addEventListener('click', function () {
        ...
        inputGroup.innerHTML = `
            <input type="text" name="financial_aid_agency[]" class="form-control" placeholder="Masukkan Nama Agensi Penyedia Bantuan Kewangan">
            <button type="button" class="btn btn-danger remove-agency" title="Buang"><i class="fa fa-trash"></i></button>
        `;
    ...
    epfContributor.addEventListener('change', toggleEpfType);
    financialAid.addEventListener('change', toggleAidAgency);

    toggleEpfType(); // on page load
    toggleAidAgency(); // on page load
});
</script>
@endpush
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select id="state_id" name="state_id" class="form-control" required>
                                                ...
                                                <option value="{{ $id }}" {{ old('state_id', $jettyData['state_id']
                                                    ?? '' )==$id ? 'selected' : '' }}>
                                                    {{ strtoupper($name) }}
                                                </option>
                                                @endforeach
    ...
    const routeDistricts = '{{ route('kadPendaftaran.permohonan-08.getDistricts','') }}';
    const routeJetties = '{{ route('kadPendaftaran.permohonan-08.getJetties','') }}';
    const routeRivers = '{{ route('kadPendaftaran.permohonan-08.getRivers','') }}';
    ...
    if (selectedStateId) {
        const finalDistrictUrl = routeDistricts+'/'+selectedStateId;
        ...
        fetch(finalDistrictUrl)
            ...
            .then(data => {
                ...
                if (selectedDistrictId) {
                    const finalJettyUrl = routeJetties+'/'+selectedDistrictId;
                    const finalRiverUrl = routeRivers+'/'+selectedDistrictId;
    ...
    stateSelect.addEventListener('change', function () {
        ...
        if (stateId) {
            const finalDistrictUrl = routeDistricts + '/' + stateId;
    ...
    districtSelect.addEventListener('change', function () {
        ...
        if (districtId) {
            const finalJettyUrl = routeJetties+'/'+districtId;
            const finalRiverUrl = routeRivers+'/'+districtId;
                    ...
                    for (const [id, name] of Object.entries(data)) {
                        riverSelect.innerHTML += `<option value="${id}">${name.toUpperCase()}</option>`;
                                            ...
                                            <div class="row mb-3 align-items-start">
                                                ...
                                                <div class="col-md">
                                                    ...
                                                    <div class="custom-file">
                                                        ...
                                                        <label class="custom-file-label" for="main_0_file">
                                                            {{ isset($main['file_path']) ? basename($main['file_path'])
                                                            : 'Pilih Fail' }}
                                                        </label>
                                                    </div>
                                                ...
                                                <div class="col-md-auto text-center">
                                                    ...
                                                    <a type="button" class="btn btn-primary"
                                                        onclick="window.open('{{ route('kadPendaftaran.permohonan-08.viewTempEquipment', ['type' => 'UTAMA', 'index' => 0]) }}', 'viewTempEquipmentWindow', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
                                                ...
                                                <div class="row mb-3 align-items-start">
                                                    ...
                                                    <div class="col-md-auto text-center">
                                                        ...
                                                        <button type="button" class="btn btn-primary"
                                                            onclick="window.open('{{ route('kadPendaftaran.permohonan-08.viewTempEquipment', ['type' => 'TAMBAHAN', 'index' => $i]) }}', 'tambahEquipmentWindow', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
                                                            <i class="fa fa-search p-1"></i>
                                                        </button>
                                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select name="has_vessel" id="has_vessel" class="form-select">
                                            ...
                                            <option value="yes" {{ old('has_vessel', $vesselData['has_vessel'] ?? ''
                                                ...
                                                YA</option>
                                            <option value="no" {{ old('has_vessel', $vesselData['has_vessel'] ?? ''
                                                )=='no' ? 'selected' : '' }}>
                                                TIDAK</option>
                                        </select>
                                    </div>
                                        ...
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <select name="hull_type" id="hull_type" class="form-select" required>
                                                    ...
                                                    <option value="{{ $name }}" {{ old('hull_type',
                                                        $vesselData['hull_type'] ?? '' )==$name ? 'selected' : '' }}>
                                                        {{ $name }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <label for="engine_brand" class="form-label">Jenama Enjin <span
                                                            class="text-danger">*</span></label>
                                                    <select class="form-select" id="engine_brand" name="engine_brand"
                                                        required>
                                                        <option value="">Sila Pilih</option>
                                                        @foreach ($engineBrand as $id => $name)
                                                        <option value="{{ $name }}" {{ old('engine_brand',
                                                            $vesselData['engine_brand'] ?? '' )==$name ? 'selected' : ''
                                                            }}>
                                                            {{ $name }}
                                                        </option>
                                                        @endforeach
                            ...
                            {{-- <div class="tab-pane fade" id="content-tab6" role="tabpanel"
                                ...
                                <form method="POST" id="store_tab6"
                                    action="{{ route('kadPendaftaran.permohonan-08.store_tab6') }}"
                                    ...
                                    <section>
                                        ...
                                        <small class="text-muted">
                                            Sila cetak permohonan anda, dapatkan tandatangan daripada Penghulu/Ketua
                                            Kampung/JKKK/JKOA/MyKP,
                                            dan muat naik semula dokumen tersebut.
                                        </small>
                                        <div class="mt-3">
                                            <a href="{{ route('kadPendaftaran.permohonan-08.printApplication') }}"
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md">
                                                ...
                                                $existingFileName = $existingSignedFile ? basename($existingSignedFile)
                                                : 'Pilih fail';
                                                @endphp
                                                ...
                                                <small class="text-muted">Format dibenarkan: PDF, JPG, PNG, DOCX. Saiz
                                                    maksimum: 2MB.</small>
                                    ...
                                    <section class="mt-5">
                                        ...
                                        <small class="text-muted">
                                            Sila muat turun borang dari menu <strong>Dokumen</strong>, lengkapkan dan
                                            dapatkan tandatangan,
                                            kemudian muat naik semula ke dalam sistem.
                                        </small>

                                        <div class="alert alert-warning mt-3">
                                            <strong>Perhatian:</strong> Muat turun borang dari menu
                                            <strong>Dokumen</strong> dahulu sebelum muat naik di sini.
                                        </div>

                                        <div class="col-md">
                                            <label for="landing_declaration_form" class="form-label">
                                                Borang Pengisytiharan Pendaratan Bertandatangan <span
                                                    class="text-danger">*</span>
                                            </label>

                                            @php
                                            $landingFilePath = $landingDoc['file_path'] ?? null;
                                            $landingFileName = $landingFilePath ? basename($landingFilePath) : 'Pilih
                                            fail';
                                            @endphp

                                            <div class="custom-file">
                                                <input type="file" name="landing_declaration_form"
                                                    id="landing_declaration_form" class="custom-file-input" required>
                        ...
                        {{-- <div class="tab-pane fade" id="content-tab7" role="tabpanel" aria-labelledby="tab7-link">
                            <form id="submitPermohonan" method="POST" enctype="multipart/form-data"
                                action="{{ route('kadPendaftaran.permohonan-08.store') }}"
                        ...
                        <div class="tab-pane fade" id="content-tab6" role="tabpanel" aria-labelledby="tab6-link">
                            <form method="POST" id="store_tab6"
                                ...
                                <section>
                                    ...
                                    @php
                                    $signedTitle = 'Borang Pengesahan Penghulu/Ketua Kampung/JKKK/JKOA/MyKP';
                                    $signedAppDoc = collect($documentsData ?? [])->firstWhere('title', $signedTitle);
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            <div style="flex-grow: 1;">
                                                <div class="custom-file">
                                                    <input type="file" name="signed_application" id="signed_application"
                                                        class="custom-file-input" accept=".pdf,.jpg,.jpeg,.png,.docx"
                                                        required>
                                            ...
                                            <a href="#" class="btn btn-primary"
                                                onclick="window.open('{{ route('kadPendaftaran.permohonan-08.viewTempDocument', ['title' => $signedTitle]) }}', 'signedDocWindow', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
                                                <i class="fa fa-search p-1"></i>
                                            </a>
                                        ...
                                        <small class="text-muted">Format dibenarkan: PDF, JPG, PNG, DOCX. Saiz maksimum:
                                            2MB.</small>
                                    </div>


                                    @php
                                    $landingTitle = 'Borang Pengisytiharan Pendaratan';
                                    $landingDoc = collect($documentsData ?? [])->firstWhere('title', $landingTitle);
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <div style="flex-grow: 1;">
                                                    <div class="custom-file">
                                                        <input type="file" name="landing_declaration_form"
                                                            id="landing_declaration_form" class="custom-file-input"
                                                ...
                                                <div class="col-md-auto text-center">
                                                    <a href="#" type="button" class="btn btn-primary"
                                                        onclick="window.open('{{ route('kadPendaftaran.permohonan-08.viewTempDocument', ['title' => $landingTitle]) }}', 'landingDocWindow', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
                                                        <i class="fa fa-search p-1"></i>
                                                    </a>
                            ...
                            <form id="submitPermohonan" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="form-check mt-4 mb-4 text-center">
                                    ...
                                    <label class="form-check-label fw-semibold text-secondary"
                                        ...
                                        Saya dengan ini mengakui dan mengesahkan bahawa semua maklumat yang diberikan
                                        oleh saya adalah benar.
