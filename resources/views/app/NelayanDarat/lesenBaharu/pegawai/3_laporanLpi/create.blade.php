...
<div id="app-content">
                ...
                <div class="col-md-3 align-content-center">
                    ...
                    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
                        <ol class="breadcrumb text-center">
                            <li class="breadcrumb-item"><a href="http://127.0.0.1:8000/lesenBaharu/laporanLpi-01">{{
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="tab-content" id="pills-tabContent">
                                            ...
                                            <div class="tab-pane fade    " id="content-tab1" role="tabpanel"
                                                ...
                                                <form id="form-tab1"
                                                    action="{{ route('lesenBaharu.laporanLpi-01.storeMaklumatVesel', $application->id) }}"
   ...
   onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', [
       'id' => $inspection->id,
       'field' => 'overall_image_path'
   ]) }}', 'inspectionDocWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
        ...
        input?.addEventListener('change', function () {
            ...
            if (label) label.textContent = fileName;
                                            ...
                                            <div class="tab-pane fade" id="content-tab2" role="tabpanel"
                                                ...
                                                <form id="form-tab2"
                                                    action="{{ route('lesenBaharu.laporanLpi-01.storeMaklumatEnjin', $application->id) }}"
                                                    method="POST" enctype="multipart/form-data">
                                                    @csrf
   ...
   onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', [
       'id' => $inspection->id,
       'field' => 'engine_image_path'
   ]) }}', 'engineDocWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search"></i>
</a>
   ...
   onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', [
       'id' => $inspection->id,
       'field' => 'engine_number_image_path'
   ]) }}', 'engineNumberWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search"></i>
</a>
                                            ...
                                            <div class="tab-pane fade" id="content-tab3" role="tabpanel"
                                                ...
                                                <form id="form-tab3"
                                                    action="{{ route('lesenBaharu.laporanLpi-01.storePeralatanKeselamatan', $application->id) }}"
                                                    method="POST" enctype="multipart/form-data">
                                                    @csrf
   ...
   onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', [
       'id' => $inspection->id,
       'field' => 'safety_jacket_image_path'
   ]) }}', 'safetyJacketWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search"></i>
</a>
        ...
        input?.addEventListener('change', function () {
            ...
            if (label) label.textContent = fileName;
                                            ...
                                            <div class="tab-pane fade" id="content-tab4" role="tabpanel"
                                                ...
                                                <form id="form-tab4"
                                                    action="{{ route('lesenBaharu.laporanLpi-01.storePeralatanMenangkapIkan', $application->id) }}"
                                                    method="POST" enctype="multipart/form-data">
                                                    @csrf
...
<section>
    ...
    <div class="form-group">
        <div class="row mb-3 align-items-start">
            <!-- Equipment Dropdown -->
            <div class="col-md-4">
                <label for="main_name" class="form-label">Peralatan <span class="text-danger">*</span></label>
                <select class="form-select" id="main_name" name="main[name]" required>
                    <option value="">Sila Pilih</option>
                    @foreach ($equipmentList as $id => $name)
                        <option value="{{ $name }}"
                            {{ old('main.name', $mainInspectionEquipment->name ?? '') == $name ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- Quantity Input -->
            <div class="col-md-1">
                <label for="main_quantity" class="form-label">Kuantiti <span class="text-danger">*</span></label>
                <input type="number" name="main[quantity]" id="main_quantity" class="form-control"
                    min="1"
                    value="{{ old('main.quantity', $mainInspectionEquipment->quantity ?? '') }}"
                    required>
            </div>
            ...
            <div class="col-md">
                <label for="main_file" class="form-label">
                    Gambar Peralatan
                    @if (empty($mainInspectionEquipment->file_path)) <span class="text-danger">*</span> @endif
                </label>

                <div class="custom-file">
                    <input type="file" class="custom-file-input" id="main_file" name="main[file]"
                        accept=".jpg,.jpeg,.png,.pdf"
                        @if (empty($mainInspectionEquipment->file_path)) required @endif>
                    <label class="custom-file-label" for="main_file">
                        {{ basename($mainInspectionEquipment->file_path ?? '') ?: 'Pilih Fail' }}
                    </label>
                </div>
            ...
            <!-- View File Button -->
            @if (!empty($mainInspectionEquipment->file_path))
                <div class="col-md-auto text-center">
    <label class="form-label d-none d-md-block">&nbsp;</label>
    <button type="button" class="btn btn-primary"
        onclick="window.open(`{{ route('lesenBaharu.laporanLpi-01.viewEquipment', [
            'id' => $mainInspectionEquipment->id,
            'type' => 'UTAMA'
        ]) }}`, 'utamaEquipmentWindow{{ $mainInspectionEquipment->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
...
@if (!empty($item->file_path ?? null))
    <div class="col-md-auto text-center">
        <label class="form-label d-none d-md-block">&nbsp;</label>
        <button type="button" class="btn btn-primary"
    onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewEquipment', ['id' => $item->id, 'type' => 'TAMBAHAN']) }}', 'tambahEquipmentWindow{{ $item->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search p-1"></i>
</button>
                ...
                if (e.target.closest('.remove-item-btn')) {
                    e.target.closest('.row').remove();
                                        ...
                                        <div class="tab-pane fade" id="content-tab5" role="tabpanel"
                                            ...
                                            <form id="form-tab5"
                                                action="{{ route('lesenBaharu.laporanLpi-01.storePengesahanPemeriksaan', $application->id) }}"
                                                method="POST" enctype="multipart/form-data">
                                                @csrf
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <div class="d-flex align-items-center gap-3">
                                                      ...
                                                      <button type="button" class="btn btn-primary"
    onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', ['id' => $inspection->id, 'field' => 'attendance_form_path']) }}', 'attendanceFormWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search"></i>
</button>
            ...
            if (label) label.textContent = fileName;
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <div class="d-flex align-items-center gap-3">
                                                       ...
                                                       <button type="button" class="btn btn-primary"
    onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', ['id' => $inspection->id, 'field' => 'vessel_image_path']) }}', 'vesselImageWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search"></i>
</button>
            ...
            if (label) label.textContent = fileName;
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <div class="d-flex align-items-center gap-3">
                                                        ...
                                                        <button type="button" class="btn btn-primary"
    onclick="window.open('{{ route('lesenBaharu.laporanLpi-01.viewInspection', ['id' => $inspection->id, 'field' => 'inspector_owner_image_path']) }}', 'ownerInspectorWindow{{ $inspection->id }}', 'width=1200,height=800,scrollbars=yes,resizable=yes'); return false;">
    <i class="fa fa-search"></i>
</button>
                            ...
                            <div class="d-flex justify-content-end gap-3">
                                ...
                                <div class="d-flex gap-3">
                                    ...
                                    <form id="form-submit-final" onsubmit="sessionStorage.removeItem('lastSavedTab')"
                                        action="{{ route('lesenBaharu.laporanLpi-01.store', $application->id) }}"
                                        method="POST">
                                        @csrf
                                    </form>
