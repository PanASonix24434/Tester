                        ...
                        <div class="card-body">
                                            ...
                                            @if (!$selectedKrus->isEmpty())
                                                ...
                                                @foreach ($selectedKrus as $kru)
                                                    <tr>
                                                        <td>{{++$count}}</td>
                                                    <td>
                                                        <a href="{{ route('kadpendaftarannelayan.cetakankad.showKru',$kru->id) }}" target="_blank">{{$kru->ic_number}}</a>
                                                    </td>
                                                        <td>{{$kru->name}}</td>
                                                        <td>
                                                            @php
                                                            ...
                                                            @if ( $kru->ssd_number == null || $kru->has_sucessfully_printed === false )
                                                                <form method="GET" action="{{ route('kadpendaftarannelayan.cetakankad.createSSD',$kru->id) }}">
                                                                    <!-- @csrf -->
                                                            ...
                                                            @elseif($kru->kewarganegaraan_status_id == $pemastautinId)
                                                                <div style="color: red;">Kru adalah pemastautin tetap. Sila cetak diatas Kad Pendaftaran MERAH.</div>
                                                            @endif
                                                        </td>
                                                        <td style="text-align: center;">
                                                            @if ( $kru->ssd_number != null && $kru->has_sucessfully_printed === null)
                                                                <a class="btn btn-secondary btn-sm" href="{{route('kadpendaftarannelayan.cetakankad.print',$kru->id)}}" target="_blank"><i class="fas fa-print"></i> Cetak</a>
                                                            @else
                                                        ...
                                                        <td>
                                                            @if ( $kru->ssd_number != null && $kru->has_sucessfully_printed === null)
                                                                <form method="POST" action="{{ route('kadpendaftarannelayan.cetakankad.updatePrinted',$kru->id) }}">
                                                                    @csrf
                            ...
                            <div class="row">
                                <div class="col-lg-12 text-lg-center mt-3">
                                    <form method="POST" action="{{ route('kadpendaftarannelayan.cetakankad.updateCompleted',$kru->kru_application_id) }}">
                                        @csrf
