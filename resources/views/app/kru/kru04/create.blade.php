                            ...
                            <div class="tab-pane tab-example-design fade show active" id="pills-javascript-behavior-design"
                                ...
                                <div class="tab-content p-4" id="myTabContent">
                                    <div class="tab-pane fade show active" id="application" role="tabpanel" aria-labelledby="application-tab">
                                        <form id="form" method="POST" action="{{ route('pembatalankadpendaftarannelayan.permohonan.store') }}">
                                            @csrf
                                            <div class="row">
                                                <!-- Vessel -->
                                                <div class="col-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <select class="form-select select2" id="selVessel" name="selVessel" autocomplete="off" height="100%" width="100%" required>
                                                            ...
                                                            @foreach ( $vessels as $vessel )
                                                                <option value="{{$vessel->id}}" data-vessel="{{ $vessel->no_pendaftaran }}" {{ App\Models\Kru\NelayanMarin::where('vessel_id',$vessel->id)->exists() ? '' : 'disabled' }} >{{ $vessel->no_pendaftaran }} {{ App\Models\Kru\NelayanMarin::where('vessel_id',$vessel->id)->exists() ? '' : '(TIADA KRU)' }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    @error('selVessel')
