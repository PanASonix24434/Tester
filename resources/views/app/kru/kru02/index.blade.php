        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6">
                    <!-- Page header -->
                    <div class="mb-5">
                        <h3 class="mb-0">Senarai Permohonan Pembaharuan Kad Pendaftaran Nelayan (Tempatan / Pemastautin Tetap)</h3>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-5 text-right">
                        <a href="{{ route('pembaharuankadpendaftarannelayan.permohonan.create') }}" class="btn btn-secondary btn-sm"><i class="fas fa-plus"></i> <span class="hidden-xs"> Tambah</span></a>
                    </div>
                </div>
            </div>
            <div>
                                    ...
                                    <table class="table mb-0 table-centered table-hover">
                                        ...
                                        @if (!empty($applications) && !$applications->isEmpty())
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{ $a->created_at->format('d/m/Y') }}</td>{{-- h:i:s A --}}
                                                    <td>{{ $a->reference_number ?? '-Tiada-' }}</td>
                                                    ...
                                                    <td>
                                                        @if ( $a->kru_application_status_id == $statusDisimpanId)
                                                            <a class="btn btn-sm btn-warning" href="{{ route('pembaharuankadpendaftarannelayan.permohonan.edit', $a->id) }}">
                                                                <i class="fas fa-edit"></i>
                                                            </a>
                                                        @else
                                                            <a class="btn btn-sm btn-primary" href="{{ route('pembaharuankadpendaftarannelayan.permohonan.show', $a->id) }}">
