        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-9">
                    <!-- Page header -->
                    <div class="mb-5">
                        <h3 class="mb-0">Kemaskini Dewan Undangan Negeri (DUN)</h3>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="text-right">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                              <li class="breadcrumb-item"><a href="{{ route('master-data.aduns.index') }}">Data Utama</a></li>
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                      ...
                      <div class="card mb-10">
                        <form id="form-parliamentSeat-update" method="POST" enctype="multipart/form-data" action="{{ route('master-data.aduns.update', $parliamentSeat->id) }}" autocomplete="off">
                            @csrf
                            ...
                            role="tabpanel" aria-labelledby="pills-javascript-behavior-design-tab">
								...
								<div class="mb-3">
									...
									<select id="parliament" class="form-control @error('parliament') is-invalid @enderror select2" name="parliament" required>
                                        <option value="">{{ __('app.please_select') }}</option>
                                        @foreach ($listParliament as $p)
                                            <option value="{{ $p->id }}"{{$p->id == $parliamentSeat->parliament_id ? ' selected' : '' }}>{{$p->parliament_code}}-{{ strtoupper($p->parliament_name) }}</option>
                                        @endforeach
                                    </select>
								</div>
								...
								<div class="mb-3">
									...
									<input type="text" id="code" name="code" value="{{ $parliamentSeat->parliament_seat_code }}" class="form-control" required />
								</div>

                                <!-- Nama DUN -->
								<div class="mb-3">
									<label for="name" class="form-label">Nama Dewan Undangan Negeri (DUN) : <span style="color:red;">*</span></label>
									<input type="text" id="name" name="name" value="{{ $parliamentSeat->parliament_seat_name }}" class="form-control" required />
								</div>
                        ...
                        <div class="col-lg-12 text-lg-center mt-3 mt-lg-0" style="margin-top:-10px;">
                            ...
                            <a href="javascript:void(0);" class="btn btn-secondary btn-sm" onclick="event.preventDefault(); document.getElementById('form-parliamentSeat-update').submit();"><i class="fas fa-save"></i> Simpan</a>
                        </div><br/>
